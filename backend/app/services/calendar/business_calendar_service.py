# Advanced Business Calendar Service - Phase B Sprint 2 Day 6
...
class BusinessCalendarService:
        ...
        try:
            ...
            insert_query = """
                ...
                ON CONFLICT (holiday_id) DO UPDATE SET
                    ...
                    affects_delivery = EXCLUDED.affects_delivery,
            ...
            logger.info(f"Created/updated holiday: {holiday.name} on {holiday.date}")
            return True
    ...
    def get_calendar_summary(self, month: int, year: int) -> Dict[str, Any]:
        """Get comprehensive calendar summary for a month"""
        ...
        holidays_in_month = self.get_holidays_in_range(start_date, end_date)
        ...
        holiday_count = len([h for h in holidays_in_month if h.affects_delivery])
        weekend_count = sum(1 for d in range(1, last_day + 1) 
        ...
        return {
            ...
            'delivery_days_available': business_day_count,
            'holidays_list': [
                {
                    'name': h.name,
            ...
            'business_days_list': [
                {
                    ...
                    'day_of_week': bd.date.strftime('%A'),
                    'delivery_allowed': bd.delivery_allowed
                } for bd in business_days
            ]
        }
    
    def get_delivery_capacity_forecast(self, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        total_capacity_days = len(business_days)
        upcoming_holidays = [h for h in holidays if h.affects_delivery]
        ...
        while current_week_start <= end_date:
            ...
            week_business_days = [bd for bd in business_days 
                                if current_week_start <= bd.date <= week_end]
            
            week_key = f"Week of {current_week_start.isoformat()}"
            weekly_breakdown[week_key] = {
                'start_date': current_week_start.isoformat(),
                'end_date': week_end.isoformat(),
                'business_days': len(week_business_days),
                'delivery_capacity': len(week_business_days) * 8  # Assuming 8 deliveries per day
            }
        ...
        return {
            ...
            'capacity_summary': {
                'total_business_days': total_capacity_days,
                'total_delivery_capacity': total_capacity_days * 8,
                'upcoming_holidays': len(upcoming_holidays),
                'average_daily_capacity': 8
            },
            'upcoming_holidays': [
                {
                    'name': h.name,
                    'date': h.date.isoformat(),
                    'impact': 'Delivery suspended' if h.affects_delivery else 'No impact'
                } for h in upcoming_holidays
            ],
            'weekly_breakdown': weekly_breakdown,
            'recommendations': self._generate_capacity_recommendations(
                business_days, upcoming_holidays
            )
        }
    ...
    def _load_business_hours(self) -> List[BusinessHours]:
        ...
        try:
            # Check if business_hours_config table exists
            check_table_query = """
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_name = 'business_hours_config'
                );
            """
            
            result = self.db.execute(text(check_table_query))
            table_exists = result.scalar()
            
            if not table_exists:
                logger.info("Business hours config table doesn't exist, using defaults")
    ...
    def _load_company_holidays(self) -> List[Holiday]:
        ...
        try:
            ...
            if not table_exists:
                logger.info("Company holidays table doesn't exist, using empty list")
                return []
    ...
    def _generate_capacity_recommendations(self, 
                                         business_days: List[WorkingDay], 
                                         upcoming_holidays: List[Holiday]) -> List[str]:
        """Generate capacity planning recommendations"""
        
        recommendations = []
        
        # Check for upcoming capacity constraints
        if len(upcoming_holidays) > 2:
            recommendations.append(
                f"High holiday impact: {len(upcoming_holidays)} holidays upcoming. "
                "Consider scheduling critical deliveries early."
            )
        
        # Check weekly distribution
        if len(business_days) < 15:  # Less than 3 weeks of business days
            recommendations.append(
                "Limited delivery window. Consider prioritizing high-importance reports."
            )
        
        # Weekend buffer recommendation
        friday_count = sum(1 for bd in business_days if bd.date.weekday() == 4)
        if friday_count < 3:
            recommendations.append(
                "Few Friday delivery opportunities. Plan weekend processing accordingly."
            )
        
        if not recommendations:
            recommendations.append("Delivery capacity looks optimal for the forecast period.")
